...
from ranking_models import _DRMF, _PMF
...
parser.add_argument("-td", "--threshold_length_document", type=float,
                    help="Threshold to control the number of sequences for each user/item (default = 0.8)", default=0.8)
...
if do_preprocess:
    ...
    for i in range(R.shape[0]):
        ...
        if len(q.ground_truth)!=0:
            q.extendSelectedItems(all_item_set.difference(set(train_user[0][i])))  
        query_list.append(q)
        
    '''PMF'''
    _PMF(max_iter=max_iter, res_dir=res_dir, lambda_u=1.0, lambda_v=5.0, dimension=dimension,
                 train_user=train_user, train_item=train_item, valid_user=valid_user, test_user=test_user, R=R, query_list=query_list)    
    ...
    _DRMF(max_iter=max_iter, res_dir=res_dir, lambda_u=0.5, lambda_v=5.0, dimension=dimension, vocab_size=vocab_size, init_W=init_W, give_weight=give_weight, DNN_X=DNN_X, DNN_Y=None,
                    ...
                    dnn_type='CNN', reg_schema='Item', maxlen_doc=[maxlen_doc_x, 0], maxlen_sent=[maxlen_sent_x, 0], query_list=query_list)
 
    
    '''DRMF-CNNGRU-Item'''
    _DRMF(max_iter=max_iter, res_dir=res_dir, lambda_u=0.5, lambda_v=5.0, dimension=dimension, vocab_size=vocab_size, init_W=init_W,
                        ...
                        dnn_type='CNN_GRU', reg_schema='Item', maxlen_doc=[maxlen_doc_x, 0], maxlen_sent=[maxlen_sent_x, 0], gru_outdim=50, dropout_rate=0.4, query_list=query_list)
    ...
    '''DRMF-CNNGRU-User'''
    _DRMF(max_iter=max_iter, res_dir=res_dir, lambda_u=5, lambda_v=0.5, dimension=dimension, vocab_size=vocab_size, init_W=init_W,
                      give_weight=give_weight, DNN_X=None, DNN_Y=DNN_Y, emb_dim=emb_dim, num_kernel_per_ws=50,
                      train_user=train_user, train_item=train_item, valid_user=valid_user, test_user=test_user, R=R,
                      dnn_type='CNN_GRU', reg_schema='User', maxlen_doc=[0, maxlen_doc_y], maxlen_sent=[0, maxlen_sent_y], gru_outdim=50, dropout_rate=0.4, query_list=query_list)
    
    '''DRMF-CNNGRU-Dual'''
    _DRMF(max_iter=max_iter, res_dir=res_dir, lambda_u=5, lambda_v=0.5, dimension=dimension, vocab_size=vocab_size, init_W=init_W,
                     give_weight=give_weight, DNN_X=DNN_X, DNN_Y=DNN_Y, emb_dim=emb_dim, num_kernel_per_ws=50,
                     train_user=train_user, train_item=train_item, valid_user=valid_user, test_user=test_user, R=R,
                     dnn_type='CNN_GRU', reg_schema='Dual', maxlen_doc=[maxlen_doc_x, maxlen_doc_y], maxlen_sent=[maxlen_sent_x, maxlen_sent_y], gru_outdim=50, dropout_rate=0.4, query_list=query_list)
