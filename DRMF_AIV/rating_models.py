...
b = 0
ENDURE_COUNT = 3
def DRMF(res_dir, train_user, train_item, valid_user, test_user,
           ...
           dnn_type='CNN_GRU', reg_schema='Dual', gru_outdim=50, maxlen_doc=[10, 10], maxlen_sent=[30, 30]):
    # explicit setting
    num_user = R.shape[0]
    num_item = R.shape[1]
    PREV_LOSS = 1e-50
    ...
    if dnn_type == 'CNN':
        if reg_schema == 'Item' or reg_schema == 'Dual':
            dnn_module_x = CNN_module(dimension, vocab_size, dropout_rate,
                                emb_dim, maxlen_doc[0] * maxlen_sent[0], num_kernel_per_ws, init_W)
        if reg_schema == 'User' or reg_schema == 'Dual':
            dnn_module_y = CNN_module(dimension, vocab_size, dropout_rate,
                                emb_dim, maxlen_doc[1] * maxlen_sent[1], num_kernel_per_ws, init_W)
            
    if dnn_type == 'CNN_GRU':
        if reg_schema == 'Item' or reg_schema == 'Dual':
            dnn_module_x = CNN_GRU_module(dimension, vocab_size, dropout_rate,
                                emb_dim, gru_outdim, maxlen_doc[0], maxlen_sent[0], num_kernel_per_ws, init_W)
        if reg_schema == 'User' or reg_schema == 'Dual':
            dnn_module_y = CNN_GRU_module(dimension, vocab_size, dropout_rate,
                                emb_dim, gru_outdim, maxlen_doc[1], maxlen_sent[1], num_kernel_per_ws, init_W)
...
#             count = count + 1
        ...
        print ("Loss: %.5f Elpased: %.4fs Converge: %.6f Tr: [%.5f, %.5f, %.5f] Val: [%.5f, %.5f, %.5f] Te: [%.5f, %.5f, %.5f] " % (
            loss, elapsed, converge, tr_mae, tr_mse, tr_rmse, val_mae, val_mse,
            val_rmse, te_mae, te_mse, te_rmse))
        f1.write("Loss: %.5f Elpased: %.4fs Converge: %.6f Tr: [%.5f, %.5f, %.5f] Val: [%.5f, %.5f, %.5f] Te: [%.5f, %.5f, %.5f]\n" % (
            loss, elapsed, converge, tr_mae, tr_mse, tr_rmse, val_mae, val_mse, val_rmse, te_mae, te_mse, te_rmse))
...
def PMF(res_dir, train_user, train_item, valid_user, test_user,
           R, max_iter=50, lambda_u=1, lambda_v=100, dimension=50, give_weight=False):
    # explicit setting
    num_user = R.shape[0]
    num_item = R.shape[1]
    PREV_LOSS = 1e-50
    ...
    for iteration in range(max_iter):
        ...
        for j in range(num_item):
            ...
            sub_loss[j] = -0.5 * a * np.square(R_j).sum()
